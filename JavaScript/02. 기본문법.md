...
- 문자열 자료형도 비교할 수 있다.  유니코드 문자의 코드 값을 기준으로 비교하므로 모든 언어를 비교할 수 있다.

```
'기린' > '나비'  //사전순으로 비교하기 때문에 false
```

- 불 자료형 끼리 비교 가능하다. true는 1, false는 0으로 변환한뒤 비교한다.
...
- 값을 저장할 때 사용하는 식별자. 숫자 자료형뿐만 아니라 모든 자료형을 저장할 수 있다.
...
- 숫자가 아닌 값을 입력했을 때는 NaN(Not a Number)값을 출력한다.
- NaN은 자바스크립트로 나타낼 수 없는 숫자를 뜻한다.
